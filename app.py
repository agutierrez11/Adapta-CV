...
if search_btn and search_query:
    if not api_key:
        st.error(f"Falta configurar la API Key de {provider.upper()} en los Secrets de Streamlit Cloud.")
    else:
        with st.spinner("Buscando vacantes en LinkedIn, Indeed y más..."):
            try:
                ...
                st.error(f"Error al buscar empleos: {e}")
...
if st.button("🔥 ANALIZAR Y GENERAR TODO", type="primary", use_container_width=True):
    ...
    elif not cv_text or not final_job_text:
