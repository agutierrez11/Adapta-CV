...
# Mostrar Resultados del Análisis (Estilo imagen)
...
if "cv" in st.session_state:
    ...
    with col_cv:
        ...
        st.download_button("Word (.docx)", DocumentGenerator.generate_word(st.session_state.cv, template), "CV_Optimizado.docx", use_container_width=True)
    ...
    with col_cl:
        ...
        st.download_button("Word (.docx)", DocumentGenerator.generate_cl_word(st.session_state.cl, st.session_state.cv.full_name, template), "Carta_Presentacion.docx", use_container_width=True)
