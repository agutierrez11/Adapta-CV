...
class CVOptimizer:
    ...
    def optimize_cv(self, cv_text: str, job_description: str, language: str = "es") -> OptimizedCV:
        ...
        IMPORTANT: You MUST return a JSON object with EXACTLY these English keys, even if the content is in {language}:
        'full_name', 'email', 'phone', 'location', 'professional_summary', 'key_skills', 'experience', 'education', 'languages', 'certifications'.
        Inside 'experience', use keys: 'job_title', 'company', 'duration', 'achievements'.
        Inside 'education', use keys: 'degree', 'institution', 'year'.
        Respond ONLY with valid JSON."""
        
        user_prompt = f"Optimize this CV for this job description. Language of content: {language}. CV: {cv_text} | Job: {job_description}"
        ...
        return OptimizedCV(**json.loads(json_str))

    def analyze_ats(self, cv_text: str, job_description: str, language: str = "es") -> ATSAnalysis:
        system_prompt = f"""You are an ATS simulator. Analyze the match.
        IMPORTANT: You MUST return a JSON object with EXACTLY these English keys:
        'match_score', 'missing_keywords', 'suggestions', 'strengths'.
        Respond ONLY with valid JSON."""
        ...
        content = response.choices[0].message.content
        json_str = content[content.find('{'):content.rfind('}')+1]
        return ATSAnalysis(**json.loads(json_str))
